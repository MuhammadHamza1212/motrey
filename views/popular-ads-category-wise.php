...
<?php
$value = $categoryService->getCategoryById(($currentLanguageDirection === "rtl" ? 319 : 1));
$adService = new AdService();

echo '<div class="heading section-heading w-100 bg-transparent" >
<div class="bg-white px-sm-2 me-sm-2 d-inline-block section-heading-text" style="width:fit-content;">'.$labels[$currentLanguageIsoCode]['Popular_In'] . ' ' .$value->getName().':'.'</div>
<div class="mx-sm-4 bg-white '.($currentLanguageDirection === "rtl" ? "float-start" : "float-end").'"><a href="ads.php?category_id='.($currentLanguageDirection === "rtl" ? 319 : 1).'" class="btn btn-outline-danger">'.$labels[$currentLanguageIsoCode]['View_All'].'</a></div>
</div>';

echo '';

$allHierarchicalSubCategories = $categoryService->getAllCategoriesHierarchicallyByParentCategoryId($value->getCategory_id());
$allHierarchicalSubCategoriesIdsSal = array($value->getCategory_id_sal());
...
$ads = $adService->getAdByCategoryIdsSal($allHierarchicalSubCategoriesIdsSal);
if($ads == NULL) {
	echo '<div class="alert alert-warning p-4 mt-4" role="alert">
			<div class="row ms-0">
				...
				<div class="col-8 col-lg-11">
				  <h4 class="alert-heading">'.$labels[$currentLanguageIsoCode]['We_could_not_find_any_ads_related_to'].' '.$value->getName().'</h4 >
				  <div>'.$labels[$currentLanguageIsoCode]['Please_come_again_later'].'</div>
				</div>
			</div>
		</div>';
}
    				...
    				echo '<div class="card-body ps-0 pt-2">
    				  ...
    				  echo '<p class="card-text mb-0">';
    							...
    							foreach($categoryAttributes as $attribute) {
    								$categoryAttributesValue = $categoryAttributesValuesService->getCategoryAttributesValueByAdIdAndAttributeId($ads[$i]->getAd_id(), $attribute->getAttribute_id());
    								echo $categoryAttributesValue->getValue() . ' ';
    							}*/
								$text = $translate->translate($ads[$i]->getTitle(), ['target' => $currentLanguageIsoCode])['text'];
    							echo $text;
    				  echo '</p>';
    				  $today = strtotime("now");
    				  ...
    				  echo '<p class="card-text text-danger" style="font-style: bold;">'. $adPostDays . '</p>
    					  </div>
    					</div>
    				</a>
    			</div>';
